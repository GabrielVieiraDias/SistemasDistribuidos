			...
			while ((line = br.readLine()) != null) {
				...
				if(content.length>0){
					...
					if(mapa.getTipoOperacaoId() == 1 || mapa.getTipoOperacaoId() == 2)
						ServerThreadDisco.mapa.put(new BigInteger(String.valueOf(mapa.getChave())), mapa.getTexto());
					else if(mapa.getTipoOperacaoId() == 3)
						ServerThreadDisco.mapa.remove(new BigInteger(String.valueOf(mapa.getChave())));
				}
